 ...
 *
 * An employee gets exactly one {@see User}: created with a generated temporary
 * password, granted the baseline `staff` role (self-service clock + leave), and
 * linked back via {@see Employee::$user_id}. Idempotent — an employee that
 * already has an account is returned untouched (with no password, since the
 * existing secret is never knowable).
 */
class EmployeeAccountProvisioner
{
    ...
    public static function provision(Employee $employee): array
    {
        ...
        // Reuse an existing account on the same email (e.g. a rehire) rather than
        // colliding on the global email-uniqueness the mobile login depends on.
