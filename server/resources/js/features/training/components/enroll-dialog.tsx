...
/**
 * Enroll an employee in a program, or edit an existing enrollment. The employee
 * picker only shows when enrolling — on an existing enrollment the employee is
 * fixed and only the status / score / remarks change.
 */
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {isEditing
                            ? `${enrollment?.employee?.full_name ?? 'This employee'}'s status in this program.`
                            : 'Add an employee to this training program.'}
                    </DialogDescription>
                </DialogHeader>
...
function FormBody({
    ...
    const submit = (event: React.FormEvent) => {
        ...
        if (isEditing && enrollment) {
            updateEnrollment(
                enrollment.id,
                { status, score: scoreValue, remarks: remarks || null },
                handlers,
            );
        } else {
            enrollEmployee(
